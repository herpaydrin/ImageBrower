...
class AHImagesBrower: UIView,UIScrollViewDelegate {
    ...
    func getAmImageBrower(index:CGFloat,tag:Int) -> AHZoomView{
        ...
        let tap = UITapGestureRecognizer.init(target: self, action: #selector(hideThisImage(tap:)))
        brower.addGestureRecognizer(tap)
        brower.tag = 100+tag
        scrollView?.addSubview(brower)
        return brower
    }
    
    func hideThisImage(tap:UITapGestureRecognizer) {
        let imageV = (tap.view as! AHZoomView).imageView
        UIView.animate(withDuration: 0.2, animations: {
            imageV?.frame = self.oldFrame!
        }) { (finsh) in
